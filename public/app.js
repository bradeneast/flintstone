(() => {
  ...
  let selectByIndex = (index, context) => $(`[data-index="${index}"]`, context);
  ...
  let refresh = () => location = location.href;
  ...
  let toggleRootClass = (className, force) => document.documentElement.classList.toggle(className, force);
  ...
  var Modal_default = (component) => html`
<modal @click=${() => toggleMenu_default(false)} class=mount>
  ${component}
</modal>`;

  // source/js/components/AuthScreens/AuthError.js
  var AuthError_default = (message = "Your email or password may be invalid") => html`
<span>
  ${message}.
  ${Button_default({
    content: "Try logging in again",
    className: "link underline",
    action: () => renderAll2(Modal_default(Login()))
  })}
  or
  ${Button_default({
...
<label>
    ...
    ${Button_default({
      ...
      action: () => {
        ...
        auth_default.requestPasswordRecovery(formData.email).then(() => {
          ...
          renderAll2(Modal_default("Recovery email sent. Check your inbox."));
        }).catch((err) => {
          console.log(err);
          state_default.error = err.message;
          setState("loading", false);
          renderAll2(Modal_default(AuthError_default(err.message)));
        });
      }
    })}
    ${Button_default({
  ...
  // source/js/components/Welcome.js
  var Welcome_default = () => html`
<div>
...
${Button_default({
    ...
    className: "primary",
    action: () => {
      state_default.savedLocally = true;
...
${Button_default({
    ...
    className: "link",
    action: () => defaultState.then((defaultState2) => {
  ...
  <form class=mount>
    ...
    <label>
      ...
      action: () => renderAll2(Modal_default(Recover_default()))
    })}
    </label>
    ...
    ${Button_default({
      className: "link",
      content: "Continue using anonymously",
      action: () => state_default.savedLocally ? renderAll2() : renderAll2(Modal_default(Welcome_default()))
    })}
  </form>`;
  }
    ...
    ${Button_default({
      ...
      action: () => {
        ...
        auth_default.signup(formData.email, formData.password, {full_name: formData.name}).then(() => {
          setState("loading", false);
          renderAll2(Modal_default("Confirmation email sent. Check your inbox."));
        }).catch((err) => {
          console.log(err);
          state_default.error = err.message;
          setState("loading", false);
...
<svg class=${className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 216 216">
  ...
  // source/js/functions/toggleStyleEditor.js
  var toggleStyleEditor_default = (force) => {
    if (force == void 0) {
  ...
  // source/js/functions/printDocument.js
  var printDocument_default = () => {
    state_default.loading = true;
    state_default.showPreview = true;
    renderAll2();
    setTimeout(() => {
      print();
      setState("loading", false);
  ...
  ${user2 ? nothing : Button_default({
      className: "primary",
      content: "Sign up",
      action: () => renderAll2(Modal_default(SignUp_default()))
    })}
  ${Button_default({
      className: "link",
      content: user2 ? html`Signed in as <strong>${user2.user_metadata.full_name || user2.email}</strong>` : "Sign in",
      action: user2 ? () => renderAll2(Modal_default(SignOut_default())) : () => renderAll2(Modal_default(Login()))
    })}
  ${state_default.savedLocally ? html`<separator></separator>` : nothing}
  ${state_default.savedLocally ? Button_default({
      title: `${state_default.showMenu ? "Close" : "Open"} menu`,
  ...
  // source/js/functions/toggleMenu.js
  var toggleMenu_default = (force) => {
    if (force == void 0) {
  ...
  // source/js/components/DocumentTOC.js
  let makePageID = (index) => `page_${index + 1}`;
  let pageSplitter = /\n\s*\-{3,}\s*\n/;
  var DocumentTOC_default = () => {
    let doc = state_default.currentDocument.body;
    let goToPage = (index) => {
      var _a;
      setState("showPreview", true);
      (_a = $("#" + makePageID(index))) == null ? void 0 : _a.scrollIntoView();
    };
  ...
  var renderPreview_default = () => {
    ...
    return sanitized.split("<hr>").map((pageContent, index) => html`
  ...
  var addDataset_default = () => {
    ...
    let length = state_default.currentUser.datasets.length;
    state_default.currentUser.datasets.push(newDataset);
    setCurrentDataset_default(length);
  ...
  var addField_default = () => {
    ...
    setState("currentDataset", state_default.currentDataset, true);
    let targetField = selectByIndex(state_default.currentDataset.fields.length - 1, $(".fields"));
  ...
  var removeDataset_default = (index) => {
    ...
    let anim = new UIAnimation(targetItem, () => {
      ...
      setState("currentDataset", state_default.currentUser.datasets[0], true);
    });
    anim.removed();
    if (state_default.showPreview)
      renderPreview_default();
  ...
  var moveField_default = (fieldIndex, amount) => {
    ...
    setState("currentDataset", state_default.currentDataset, true);
    let anim = new UIAnimation([currentField, targetField], () => $(`[title*="${direction}"]`, targetField).focus());
    anim.swapped(amount);
  };
  ...
  <li data-index=${fieldIndex}>
    <key>
      ...
      .value=${key} 
      id=${keyID}
      @input=${(event) => renameField_default(fieldIndex, event.target.value)} />
    </key>
    <value>
      <input 
      ...
      .value=${value} 
      id=${valueID}
      @input=${(event) => updateFieldValue_default(fieldIndex, event.target.value)} />
    </value>
    <actions>
...
<div @click=${() => toggleStyleEditor_default(false)} class="pane data">
  ...
  ${state_default.currentDataset ? html`
    <div class="selection">
      ...
      <h2 data-active=true>
        ...
        type=text 
        .value=${state_default.currentDataset.id} 
        @input=${(event) => renameDataset_default(event.target.value)} />
      </h2>
...
`;
  ...
  var addDocument_default = () => {
    ...
    let length = state_default.currentUser.documents.length;
    state_default.currentUser.documents.push(newDocument);
    setCurrentDocument_default(length);
  ...
  var removeDocument_default = (index) => {
    ...
    let anim = new UIAnimation(targetItem, () => {
      ...
      setState("currentDocument", state_default.currentUser.documents[0], true);
    });
    anim.removed();
    if (state_default.showPreview)
      renderPreview_default();
  ...
  var renameDocument_default = (newName) => {
    ...
    setState("currentDocument", state_default.currentDocument, true);
  };
  ...
  var duplicateDocument_default = (index) => {
    ...
    let targetItem = selectByIndex(state_default.currentUser.documents.length - 1, $(".documents .sets"));
    let anim = new UIAnimation(targetItem);
    anim.added();
    if (state_default.showPreview)
      renderPreview_default();
...
<div class="pane documents">
  ...
  ${state_default.currentDocument ? html`
    <div class="selection">
      <h2 data-active=true>
        ...
        .value=${state_default.currentDocument.id} 
        @input=${(event) => renameDocument_default(event.target.value)} />
      </h2>
      ${state_default.currentDocument.body.length ? DocumentTOC_default() : nothing}
    </div>
    ` : nothing}
  ...
  ${state_default.showPreview ? html`
      ...
      @input=${(event) => {
      state_default.currentDocument.body = event.target.value;
      autoSave();
    }}>${state_default.currentDocument.body}</textarea>`}`;
  };
  ...
  var setPreviewStyle_default = (tagName, propName, propValue) => {
    ...
    setState("currentUser", state_default.currentUser, true);
  };
...
<div class="styles">
  ...
  <actions>
  ${state_default.expandedAdjustments.length ? Button_default({
  ...
  ${Button_default({
    ...
    action: () => toggleStyleEditor_default(false)
  })}
  </actions>
  </div>
...
<main @click=${() => toggleMenu_default(false)}>
  <section @click=${() => toggleStyleEditor_default(false)} class="aside left">${DocumentsPane_default()}</section>
  <section @click=${() => toggleStyleEditor_default(false)} class="center">${PreviewPane_default()}</section>
  ...
  let state41 = ls("flintstone_data") || {
    currentUser: {
  ...
  var state_default = state41;
  let identityState = user == null ? void 0 : user.getUserData();
  let defaultState = fetch("/defaults.json").then((r) => r.json());
  let preferences = ls("flintstone_preferences") || {
    dark: false,
    showStyles: false,
    showMenu: false
  };
  function prepState() {
    state41.currentDataset = state41.currentUser.datasets[0];
    state41.currentDocument = state41.currentUser.documents[0];
    state41.savedLocally = state41.savedLocally || false;
    state41.showPreview = state41.showPreview || false;
    state41.showStyles = state41.showStyles || false;
    state41.expandedAdjustments = state41.expandedAdjustments || ["global", "pages"];
    state41.currentUser.styles = state41.currentUser.styles || {};
    ensureProps(Object.keys(tags), state41.currentUser.styles);
    updatePreferenceClasses();
  }
  function autoSave(immediate = false) {
    state41.savedLocally = true;
    let waitAmount = 2e3;
    ...
    function save() {
      if (auth_default.currentUser())
        return auth_default.currentUser().update({data: {flintstone: JSON.stringify(state41.currentUser)}}).then(() => {
          state41.error = false;
  ...
  function renderAll2(contents = Main_default()) {
    render(html`
  <div id=app ?data-loading=${state41.loading}>
    <header>${Header_default()}</header>
    ${contents}
    <loader></loader>
    <status-message>
      ${state41.error ? `Error: ${state41.error}` : "All changes saved"}
    </status-message>
  </div>`, document.body);
  }
  ...
  function setState(key, value, save) {
    state41[key] = value;
    renderAll2();
    if (save)
      autoSave();
  ...
  <form class=mount>
    ...
    ${Button_default({
      ...
      action: () => {
        setState("loading", true);
        auth_default.currentUser().update({password}).then(() => {
          setState("loading", false);
          autoSave(true);
        }).catch((err) => {
          console.log(err);
  ...
  let completeLoading = () => {
    ...
    if (location.hash && location.hash.length) {
      ...
      if (key && value) {
        switch (key) {
          case "recovery_token":
            auth_default.recover(value, true).then(() => renderAll2(Modal_default(ResetPassword_default())));
            break;
          case "confirmation_token":
            ...
            break;
          case "invite_token":
            renderAll2(Modal_default(AcceptInvite_default(value)));
            break;
        }
      }
    } else {
  ...
  if (identityState)
    ...
    state_default.loading = false;
    renderAll2(Modal_default(Welcome_default()));
  }
})();

...
<div ?data-active=${state_default.showStyles} class="styles">
  ...
  // source/js/state.js
  let state30 = ls("contractly_user") || {};
  var state_default = state30;
  ...
  function autoSave() {
    ...
    autoSaveWaiter = setTimeout(() => {
      state30.savedLocally = true;
      ls("contractly_user", state30);
    }, 1e3);
  }
  function renderAll() {
  ...
  if (!state_default.savedLocally) {
    document.documentElement.classList.add("loading");
    defaultState.then((defaultState2) => {
      Object.keys(defaultState2).map((key) => state_default[key] = defaultState2[key]);
      ...
      Object.keys(tags).map((key) => state_default.styles[key] = state_default.styles[key] || {});
      renderAll();
      autoSave();
    ...
    state_default.dark = state_default.dark || false;

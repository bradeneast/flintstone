    ...
    var GoTrue2 = /* @__PURE__ */ function() {
      ...
      _createClass(GoTrue3, [{
        ...
        value: function createUser(tokenResponse) {
          ...
          return user.getUserData().then(function(user2) {
            if (remember) {
              user2._saveSession();
            }
            return user2;
...
`;
  ...
  var PreviewPane_default = () => {
    ...
    let previewStyles = `\r
  <style>${Object.entries(state_default.styles).map(([tagName, prop]) => `.preview ${tagName} {\r
...
`;
  ...
  var resetAdjustmentStyles_default = (tagName) => {
    defaultState.then((defaultState2) => {
      state_default.styles[tagName] = deepCopy(defaultState2.styles[tagName]);
      setState("styles", state_default.styles);
    });
  };
...
<span>
  ...
  <div class=form>
    ${Button_default({
      ...
      action: () => {
        setState("loading", true);
        auth_default.currentUser().logout().then(() => setState("loading", false));
      }
    })}
    ${Button_default({
      className: "link",
  ...
  // source/js/components/Signup.js
  var Signup_default = () => {
    let full_name;
    let email;
    let password;
  ...
  <div class=form>
    ...
    ${Button_default({
      ...
      action: () => {
        setState("loading", true);
        auth_default.signup(email, password, {full_name}).then(() => auth_default.login(email, password, true)).then((response) => {
          console.log(response);
          setState("loading", false);
        });
      }
  ...
  <nav>
    ...
    ${Button_default({
      title: `Switch to ${state_default.dark ? "light" : "dark"} theme`,
      content: state_default.dark ? "🌝" : "🌞",
      className: "icon",
      action: () => setPreference("dark", !preferences.dark)
    })}
    <separator></separator>
    ${user ? nothing : Button_default({
      className: "primary",
      content: "Sign up",
  ...
  // source/js/components/AcceptInvite.js
  ...
  <div class=form>
    ...
    ${Button_default({
      ...
      action: () => {
        setState("loading", true);
        auth_default.acceptInvite(token, password, true).then((response) => console.log(response)).then(() => auth_default.currentUser().update({full_name})).then((response) => {
          console.log(response);
          setState("loading", false);
          location.hash = "";
        });
  ...
  // source/js/components/ResetPassword.js
  ...
  if (!state_default.savedLocally) {
    defaultState.then((defaultState2) => {
      ...
      ensureProps(Object.keys(tags), state_default.styles);
  ...
  if (location.hash && location.hash.length) {
    ...
    switch (key) {
      ...
      case "confirmation_token":
        auth_default.confirm(value, true);
        break;
      case "invite_token":
